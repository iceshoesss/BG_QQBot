...
| `我的绑定` | 查看当前绑定状态 |
| `解绑` | 取消绑定，不再接收 @ 通知 |
| Webhook | 接收超时/掉线对局通知，自动 @ 已绑定玩家 |

> 以下命令已停用（代码在 `disabled_plugins/`）：排行榜、战绩、队列、最近对局、帮助
...
当前版本：`v0.1.1`（定义在 `pyproject.toml`）

## 更新日志

### v0.1.1 (2026-04-17)
- 修复 webhook 路由返回 422：添加 `fastapi.Request` 类型注解
- webhook 通知格式优化：玩家名后跟 @，而非只显示 @
- 新增「解绑」命令，取消绑定后不再接收 @ 通知
- 新增「帮助」命令，动态显示所有可用命令
